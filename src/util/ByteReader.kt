...
class ByteReader {
    companion object {
        ...
        fun readBytesToInt(size: Int, bytes: ByteArray, from: Int = 0): Int {
            ChunkReader.currentIndex += size
            val chunk =  ByteBuffer.wrap(slice(from, from + size, bytes))
            return BigInteger(chunk.array()).toInt()
        }

        fun readBytesToString(size: Int, bytes: ByteArray, from: Int = 0, encoding: Charset = StandardCharsets.UTF_8): String {
            ChunkReader.currentIndex += size
